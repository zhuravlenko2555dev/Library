...
import com.zhuravlenko2555dev.library.fragment.RetainedReadersInLibraryLogFragment;
...
public class ReadersInLibraryLogActivity extends AppCompatActivity {
    private SwipeRefreshLayout swipeRefreshLayout;
    private TableLayout tableLayoutReadersInLibrary;

    private FragmentManager fragmentManager;
    private RetainedReadersInLibraryLogFragment retainedReadersInLibraryLogFragment;

    private ArrayList<ReadersInLibraryLogRow> rowList;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_readers_in_library_log);
        ...
        retainedReadersInLibraryLogFragment = (RetainedReadersInLibraryLogFragment) fragmentManager.findFragmentByTag(RetainedReadersInLibraryLogFragment.FRAGMENT_TAG);

        if (retainedReadersInLibraryLogFragment == null) {
            retainedReadersInLibraryLogFragment  = new RetainedReadersInLibraryLogFragment();
            fragmentManager.beginTransaction().add(retainedReadersInLibraryLogFragment, RetainedReadersInLibraryLogFragment.FRAGMENT_TAG).commit();
        }
                ...
                switch (intent.getAction()) {
                    ...
                    case APIAsyncTask.RESPONSE_OK:
                        ...
                        switch (intent.getStringExtra(APIAsyncTask.EXTRA_REQUEST_ACTION)) {
                            case APIAsyncTask.ACTION_GET_READERS_IN_LIBRARY_LOG:
    ...
    protected void onResume() {
        ...
        if (retainedReadersInLibraryLogFragment.getRowList().size() != 0) {
            rowList = retainedReadersInLibraryLogFragment.getRowList();

            for (int i = 0; i < rowList.size(); i++) {
                LinearLayout tableRow = (LinearLayout) View.inflate(ReadersInLibraryLogActivity.this, R.layout.row_readers_in_library_log, null);
                ...
                ((TextView) tableRow.findViewById(R.id.textViewStatus)).setText(rowList.get(i).getStatus());
                ((TextView) tableRow.findViewById(R.id.textViewDate)).setText(rowList.get(i).getDate());
                tableLayoutReadersInLibrary.addView(tableRow);

                if (i < rowList.size()) {
                    View delimeter = new View(ReadersInLibraryLogActivity.this);
    ...
    protected void onDestroy() {
        ...
        retainedReadersInLibraryLogFragment.setRowList(rowList);
    }
    ...
    public void onBackPressed() {
        ...
        fragmentManager.beginTransaction().remove(retainedReadersInLibraryLogFragment).commit();
    }
    ...
    private void executeIntentRow() {
        ...
        Intent intentRow = new Intent(this, APIAsyncTask.class)
                .setAction(APIAsyncTask.ACTION_GET_READERS_IN_LIBRARY_LOG);
        ...
        protected Void doInBackground(Void... voids) {
            ...
            for (int i = 0; i < responseJsonArray.length(); i++) {
                try {
                    ...
                    String status = (String) responseJsonObject.get("bes_name");
                    String date = (String) responseJsonObject.get("gol_date");

                    rowList.add(new ReadersInLibraryLogRow(accountName, accountSurname, accountMiddleName, bookName, bookEditionLanguage, bookEditionPublisher, status, date));
                } catch (JSONException e) {
                    e.printStackTrace();
                }
            }
        ...
        protected void onPostExecute(Void aVoid) {
            ...
            if (!isCancelled()) {
                ...
                for (int i = 0; i < rowList.size(); i++) {
                    ...
                    ((TextView) tableRow.findViewById(R.id.textViewBookEditionLanguage)).setText(rowList.get(i).getBookEditionLanguage());
                    ((TextView) tableRow.findViewById(R.id.textViewBookEditionPublisher)).setText(rowList.get(i).getBookEditionPublisher());
                    ((TextView) tableRow.findViewById(R.id.textViewStatus)).setText("Status: " + rowList.get(i).getStatus());
                    ((TextView) tableRow.findViewById(R.id.textViewDate)).setText("Date: " + rowList.get(i).getDate());
